...
from sde_lib import VESDE, VPSDE, BRATSDE
...
def get_sde_loss_fn(config, sde, train, reduce_mean=True, continuous=True, likelihood_weighting=True, eps=1e-5):
    ...
    def loss_fn(model, health, tumor):
        ...
        t = torch.rand(
            health.shape[0], device=health.device) * (sde.T - eps) + eps
        # z is Gaussian noise and represents the log gradient in the model
        z = torch.randn_like(health) 

        mean, std = sde.marginal_prob(health, t) 
        
        perturbed_data = mean + std[:, None, None, None] * z
        ###################### TODO ######################
        if isinstance(sde, BRATSDE) and config.training.joint:
            perturbed_data = torch.cat((perturbed_data, tumor), 1) 
        ###################### TODO ######################
            
        score = score_fn(perturbed_data, t) 
        if not likelihood_weighting:
            if not config.training.joint:
                if isinstance(sde, BRATSDE) and config.training.stage == 'mri':
                    losses = torch.square(score - tumor - std[:, None, None, None] * z)
                else:
                    losses = torch.square(score * std[:, None, None, None] + z)
            ###################### TODO ######################
            else:
                if isinstance(sde, BRATSDE) and not config.model.channel_merge:
                    z = torch.cat((z, z), 1)
                    losses = torch.square(score * std[:, None, None, None] + z)
                else:
                    losses = torch.square(score * std[:, None, None, None] + z) # usecat
                    # print('losses',losses.shape)
            ###################### TODO ######################
            losses = reduce_op(losses.reshape(losses.shape[0], -1), dim=-1)
        else:
            g2 = sde.sde(torch.zeros_like(health), t)[1] ** 2
...
def get_step_fn(config, sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
    ...
    def step_fn(state, health, tumor):
        ...
        if train:
            ...
            optimizer.zero_grad()
            loss = loss_fn(model, health, tumor) # step 1
            loss.backward()
            ...
            with torch.no_grad():
                ...
                loss = loss_fn(model, health, tumor)
